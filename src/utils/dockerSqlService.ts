...
export class DockerSqlService {
  ...
  private getSpawnOptions(): any {
    return {
      stdio: "pipe",
      // Importante: manter shell=false para preservar corretamente os argumentos
      // com espaços (ex.: "-Q SELECT 1") no Windows. Usar shell=true faz o
      // cmd.exe redividir argumentos e quebra o sqlcmd, gerando erros como
      // "Sqlcmd: '1': Unknown Option".
      shell: false,
    };
  }
    ...
    try {
      ...
      for (let i = 0; i < tableNames.length; i++) {
        ...
        try {
          ...
          log.info("Tabela extraída", {
            tableName,
            rows: tableData.rows.length,
          });
        } catch (error) {
          Logger.warn(
            `Erro ao extrair ${tableName}`,
            error,
            "DockerSqlService"
          );
        }
  ...
  private async startSqlContainer(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      const dockerArgs = [
        ...
        "ACCEPT_EULA=Y",
        "-e",
        `SA_PASSWORD=${this.SQL_PASSWORD}`,
        "-e",
        "MSSQL_PID=Express", // Usar SQL Server Express (mais leve)
        "-e",
        "MSSQL_MEMORY_LIMIT_MB=1024", // Limitar memória SQL Server
        "-e",
        `MSSQL_TCP_PORT=${this.SQL_PORT}`,
        "-p",
        `${this.SQL_PORT}:1433`, // expõe porta padrão 1433 no host
      ...
      console.log(
        "Primeira execução pode demorar mais (download da imagem)..."
      );
  ...
  private async waitForSqlServer(
    onProgress?: (progress: ConversionProgress) => void
  ): Promise<void> {
    ...
    console.log(
      "Aguardando 30 segundos para garantir inicialização completa..."
    );
    await this.sleep(30000); // 30 segundos para garantir

    // Tentativas com backoff exponencial
    for (let i = 0; i < 60; i++) {
      // Reduzir para 60 tentativas de 2s cada = 2 minutos
      try {
        ...
        console.log(
          `Tentativa ${i + 1}/60 - Aguardando SQL Server... (${waitTime}ms)`
        );
        ...
        if (i % 10 === 0) {
          try {
            ...
            throw new Error(
              `Container SQL Server parou de funcionar: ${healthError}`
            );
          }
        }
    ...
    throw new Error(
      "SQL Server não conseguiu inicializar. Verifique os logs acima para detalhes."
    );
  }
  ...
  private async copyBakToContainer(filePath: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      const dockerCp = spawn(
        "docker",
        [
          "cp",
          sourcePath,
          `${this.CONTAINER_NAME}:/var/opt/mssql/data/${fileName}`,
        ],
  ...
  private async restoreDatabase(bakFileName: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        `🎯 Arquivos lógicos encontrados: DATA=${dataFile}, LOG=${logFile}`
      );
  ...
  private parseFileList(fileListOutput: string): {
    dataFile: string;
    logFile: string;
  } {
    ...
    if (fullText.includes("sicnet")) {
      ...
      logFile = "SICNET_log";
      console.log(
        `✅ Usando nomes específicos: DATA=${dataFile}, LOG=${logFile}`
      );
  ...
  private async verifyRestore(): Promise<void> {
    try {
      ...
      console.log(
        `✅ Verificação: ${count} tabelas encontradas no banco restaurado`
      );
      ...
      if (count === 0) {
        ...
        if (altLines.length > 0) {
          console.log(
            `✅ Encontradas ${altLines.length} tabelas via método alternativo`
          );
          return; // Sucesso
        }

        throw new Error(
          "Nenhuma tabela encontrada no banco restaurado (verificação dupla)"
        );
      }
    } catch (error) {
  ...
  private parseTabularResult(result: string): Record<string, any>[] {
    ...
    for (let i = headerIndex + 1; i < lines.length; i++) {
      ...
      if (
        ...
        line.startsWith("---") ||
        line.startsWith("(") ||
        line.startsWith("Changed database")
      )
        continue;
    ...
    return new Promise((resolve, reject) => {
      ...
      const sqlcmd = spawn(
        ...
        [
          ...
          "-l",
          String(timeoutSeconds), // timeout configurável
          "-s",
          "\t", // separador TAB
  ...
  private async checkContainerHealth(): Promise<void> {
    return new Promise((resolve, reject) => {
      const inspectCmd = spawn(
        "docker",
        ["inspect", this.CONTAINER_NAME, "--format={{.State.Status}}"],
        this.getSpawnOptions()
      );
  ...
  private async diagnoseSqlServerIssue(): Promise<void> {
    ...
    try {
      ...
      const statusResult = await this.runDockerCommand([
        "inspect",
        this.CONTAINER_NAME,
        "--format",
        "{{.State.Status}}",
      ...
      const logsResult = await this.runDockerCommand([
        "logs",
        "--tail",
        "20",
        this.CONTAINER_NAME,
